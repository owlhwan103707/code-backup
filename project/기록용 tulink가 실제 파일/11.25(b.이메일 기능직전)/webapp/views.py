...
def reserve_tutor_view(request, user_id):
    """
    신청하기 버튼을 누른 유저의 정보를 가져오는 뷰
    """
    try:
        # user_id를 사용해 해당 유저 정보를 가져옴
        ...
        print(f"로그인 유저야!!: {current_user.name}")
        ...
        print(f"유저 현재 전공: {user.my_current_major}")
        print(f"유저 튜터링 전공: {user.my_tutoring_major}")
        print(f"유저 세부 전공: {user.my_tutoring_sub_major}")
        print(f"유저 가능한 요일: {user.my_available_days}")
        print(f"유저 가능한 시간대: {user.my_available_times}")

        # 현재 날짜를 기반으로 오늘의 요일 가져오기
        ...
        # 유저의 my_available_times 데이터를 나누기
        user_available_times = user.my_available_times.splitlines()  # 줄 단위로 분리
        today_time_slots = []  # 오늘 날짜에 해당하는 시간대를 저장할 리스트

        for entry in user_available_times:  # 줄 단위로 처리
            try:
                day, time_slots = entry.split(",")  # 요일과 시간대를 분리
                if day.strip() == today_korean:  # 오늘의 요일과 일치하는지 확인
                    today_time_slots.extend(time_slots.split(";"))  # 해당 시간대를 리스트에 추가
            except ValueError:
                print(f"유효하지 않은 시간 데이터: {entry}")  # 잘못된 형식 데이터 출력
                continue

        # 결과 확인용 프린트
        print(f"오늘의 요일: {today_korean}")  # 오늘의 한글 요일 출력
        print(f"오늘 시간대 리스트: {today_time_slots}")  # 오늘에 해당하는 시간대 리스트 출력
        ...
        # 템플릿으로 유저 데이터를 전달
        context = {
            'tutor': user,  # 템플릿에서 tutor 변수로 유저 정보 사용 가능
            'today_time_slots': today_time_slots,  # 오늘 날짜 기준으로 필터링된 시간대 리스트
            'current_user': current_user,  # 현재 로그인한 사용자 정보

        }
        return render(request, 'webapp/reserve_tutor.html', context)

    except User.DoesNotExist:
        # 유저를 찾지 못했을 경우 에러 처리
        print(f"유저 ID {user_id}에 해당하는 유저가 존재하지 않습니다.")
        return HttpResponse("해당 유저를 찾을 수 없습니다.", status=404)
