...
def signup_step3(request):
    ...
    if request.method == 'POST':
        ...
        available_times = {}  # 요일별 시간대 저장용 딕셔너리

        # 시간대 데이터 처리
        for key, values in request.POST.lists():
            if key.startswith('available_times['):  # 키가 available_times로 시작하는 경우
                index = key.split('[')[1].split(']')[0]  # 인덱스 추출
                day = available_days[int(index)]  # 해당 인덱스에 해당하는 요일
                available_times[day] = values  # 요일과 시간대를 매핑하여 저장
        ...
        user.my_available_days = "\n".join(available_days)  # 요일 데이터를 줄바꿈으로 저장
        user.my_available_times = times_result  # 시간대 데이터를 저장
        user.save()
        ...
        tulink_data = UserTulink.objects.first()
        context = {
            'available_days': tulink_data.available_days.split(",") if tulink_data and tulink_data.available_days else [],
...
def do_tulink2(request):
    ...
    # 선택된 전공에 맞는 서브 전공 필터링 (같은 알파벳으로 시작하는 항목들)
    sub_majors = []
    if tulink_data and tulink_data.tutoring_sub_major:
        all_sub_majors = tulink_data.tutoring_sub_major.split("\n")
        if selected_major:
            # 선택된 전공의 첫 알파벳으로 필터링
            first_letter = selected_major[0]
            filtered_majors = [sub for sub in all_sub_majors if sub.startswith(first_letter)]

            # 쉼표로 분리
            for major in filtered_majors:
                sub_majors.extend(major.split(","))
...
def do_tulink3(request):
    if request.method == 'POST':
        ...
        tulink_entry.available_times = ";".join(selected_times)  # 선택된 시간대를 세미콜론으로 연결
        tulink_entry.save()
