...
def reserve_tutor_view(request, user_id):
    try:
        ...
        if request.method == 'POST':
            ...
            send_mail(
                subject='Tulink 예약 요청 알림',
                message=(
                    f"{current_user.name}님이 다음과 같이 예약을 요청했습니다.\n\n"
                    f"- 예약 시간: {selected_time_slot}\n"
                    f"- 질문: {question}\n\n"
                    f"예약자의 전공: {current_user.my_current_major}\n"
                    f"예약자의 세부 전공: {current_user.my_tutoring_sub_major}"
                ),
            ...
            send_mail(
                subject='Tulink 예약 완료 알림',
                message=(
                    f"다음과 같이 예약이 완료되었습니다.\n\n"
                    f"- 튜터 이름: {tutor.name}\n"
                    f"- 튜터 전공: {tutor.my_current_major}\n"
                    f"- 예약 시간: {selected_time_slot}\n\n"
                    f"예약이 성공적으로 완료되었습니다."
                ),
                from_email='noreply@tulink.com',
        ...
        user_available_times = tutor.my_available_times.splitlines()
        today_time_slots = []
        for entry in user_available_times:
            try:
                day, time_slots = entry.split(",")
                if day.strip() == today_korean:
                    today_time_slots = time_slots.split(";")
            except ValueError:
                continue
