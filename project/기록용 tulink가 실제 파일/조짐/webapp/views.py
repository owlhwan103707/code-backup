...
def mypage_view(request):
    # 세션에서 user_id를 가져와 사용자 정보 확인
    user_id = request.session.get('user_id')
    if not user_id:
        return redirect('webapp:login_view')  # 로그인하지 않은 경우 로그인 페이지로 이동
    ...
    context = {
        ...
        'user_major': user.my_current_major,
        'user_tutoring_major': user.my_tutoring_major,
        'user_available_days': user.my_available_days,
        'user_available_times': user.my_available_times,
    }
