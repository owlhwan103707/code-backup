...
from django.core.mail import send_mail  # 이메일 전송 기능 사용
...
def reserve_tutor_view(request, user_id):
    try:
        ...
        if not current_user_id:  # 로그인하지 않은 경우
            return redirect('webapp:login_view')
        ...
        if request.method == 'POST':
            ...
            if not question.strip():
                return HttpResponse("질문 사항을 작성해주세요.", status=400)

            # 이메일 전송
            try:
                # 튜터에게 이메일 전송
                send_mail(
                    subject='[Tulink] 튜터링 예약 요청 알림',
                    message=(
                        f"안녕하세요, {tutor.name}님.\n\n"
                        f"{current_user.name}님이 튜터링 예약을 요청하셨습니다.\n\n"
                        f"예약 정보:\n"
                        f"- 전공: {current_user.my_current_major}\n"
                        f"- 세부 전공: {current_user.my_tutoring_sub_major}\n"
                        f"- 날짜: {today_korean}\n"
                        f"- 시간: {selected_time}\n\n"
                        f"질문사항:\n{question}\n"
                    ),
                    from_email='noreply@tulink.com',
                ...
                # 예약자에게 이메일 전송
                send_mail(
                    subject='[Tulink] 예약 완료 확인',
                    message=(
                        f"안녕하세요, {current_user.name}님.\n\n"
                        f"다음과 같이 예약이 완료되었습니다:\n\n"
                        f"튜터 정보:\n"
                        f"- 전공: {tutor.my_current_major}\n"
                        f"- 세부 전공: {tutor.my_tutoring_sub_major}\n"
                        f"- 날짜: {today_korean}\n"
                        f"- 시간: {selected_time}\n\n"
                        f"Tulink를 이용해주셔서 감사합니다."
                    ),
                    from_email='noreply@tulink.com',
                ...
                # 이메일 전송 실패 시 처리
                print(f"이메일 전송 실패: {e}")
                return HttpResponse("이메일 전송에 실패했습니다. 관리자에게 문의하세요.", status=500)

            # 완료 페이지로 리디렉션
            return redirect('webapp:complete')

        # GET 요청 처리: 오늘 시간대 필터링
        tutor_available_times = tutor.my_available_times.splitlines()
        today_time_slots = []
        for entry in tutor_available_times:
            try:
                day, time_slots = entry.split(",")
                if day.strip() == today_korean:
                    today_time_slots.extend(time_slots.split(";"))
            except ValueError:
                continue

        context = {
            'tutor': tutor,
            'today_time_slots': today_time_slots,
