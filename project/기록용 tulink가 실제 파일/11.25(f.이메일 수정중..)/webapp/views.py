    ...
    try:
        ...
        if request.method == 'POST':
            ...
            send_mail(
                ...
                recipient_list=[current_user.email],  # 튜터의 이메일 주소
                fail_silently=False,  # 실패 시 예외 발생
            )

            # 튜터에게 예약 알림 이메일 전송
            send_mail(
                subject='Tulink 예약 완료 알림',  # 이메일 제목
                message=(
                    f"- 안녕하십니까 Tulink를 이용해주셔서 감사합니다 {tutor.name}님\n"
                    f"다음과 같이 예약이 완료되었습니다.\n\n"
                    f"- 튜터 이름: {current_user.name}\n"
                    f"- 튜터 전공: {tutor.my_current_major}\n"
                    f"- 예약 시간: {selected_time_slot}\n\n"
                    f"예약이 성공적으로 완료되었습니다."
                ),
                from_email='noreply@tulink.com',  # 발신자 이메일
                recipient_list=[tutor.email],  # 예약자의 이메일 주소
                fail_silently=False,  # 실패 시 예외 발생
            )
